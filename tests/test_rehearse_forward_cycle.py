...
def test首期隔离演练覆盖三策略且不修改正式账本():
    ...
    assert list(report["strategies"]) == ["v1", "v2", "v3"]
    assert [report["strategies"][key]["max_holdings"] for key in report["strategies"]] == [2, 3, 4]
    assert all(
        report["strategies"][key]["isolated_journal"]["signal_count"] == 1
        and report["strategies"][key]["isolated_journal"]["execution_count"] == 1
        and report["strategies"][key]["execution"]["residual_cash_below_one_lot"] is True
