...
- 模拟盘采用只追加账本：五策略已在 2026-08-31 收盘后生成第一期信号，当前
  均等待 2026-09-01 模拟执行。V5 从 169 只动态候选中选出 `600066`、`000651`，
  风险阀门目标为 90.22%；执行前五个账户仍各持有 10 万元现金，收益为 0%。
  高息动量策略在下一真实交易日收盘模拟执行，V2.2 在当日开盘模拟成交并按
  收盘估值。V1 对策略账户按 100% 目标投入，不设置额外现金保留；实际成交
...
- GitHub Actions 在每个工作日北京时间 18:30 首次运行，此后每小时做同日
  幂等重试至 22:30；每次先校验五策略冻结合同和交易日历，再要求 510300
  已有当日收盘价。自动任务以北京时间 18:00 为收盘数据可用边界；若 GitHub
  定时队列延迟到次日 18:00 前才启动，仍处理前一日，避免把凌晨误当成新交易日。
  任一门禁不满足都不会发布旧数据冒充新数据。
